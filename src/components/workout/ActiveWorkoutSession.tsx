...
export function ActiveWorkoutSession({
  ...
  // Restaurar progresso salvo ao carregar a sessão
  useEffect(() => {
    ...
    try {
      ...
      console.error("Erro ao restaurar progresso do treino:", error);
    }
  }, [storageKey]);

  // Salvar progresso sempre que algo relevante mudar
  useEffect(() => {
    if (typeof window === "undefined") return;
  ...
  const finishWorkout = async () => {
    try {
      ...
      if (sessionData?.mood) {
        try {
          const { data: motivationData } = await supabase.functions.invoke("generate-workout-motivation", {
            body: {
      ...
      for (const [exerciseId, exerciseProgress] of Object.entries(progress)) {
        if (exerciseProgress.completedSets > 0) {
          await supabase.from("exercise_sessions").insert({
            ...
            weight_used:
              exerciseProgress.weights.length > 0
    ...
    // Limpar progresso salvo ao finalizar o treino
        ...
        <Card className="border-primary bg-primary/5">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <Button variant="outline" size="sm" className="mt-4" onClick={() => setIsResting(false)}>
                Pular descanso
              </Button>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {currentExercise.notes && (
              <p className="text-sm text-muted-foreground bg-muted p-3 rounded-lg">💡 {currentExercise.notes}</p>
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input type="number" placeholder={currentExercise.reps} id={`reps-${currentExercise.id}`} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {currentProgress.completedSets < currentExercise.sets && (
                <Button variant="outline" className="w-full col-span-2" size="lg" onClick={skipExercise}>
