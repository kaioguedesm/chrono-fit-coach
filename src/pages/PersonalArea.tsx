...
export default function PersonalArea() {
  ...
  const handleApprove = async (workoutId: string) => {
    try {
      ...
      toast.success('Treino aprovado com sucesso!');
      fetchWorkouts();
    } catch (error) {
      console.error('Error approving workout:', error);
      toast.error('Erro ao aprovar treino');
    }
  };

  const handleReject = async () => {
    if (!selectedWorkout || !rejectionReason.trim()) {
    ...
    try {
      ...
      toast.error('Erro ao rejeitar treino');
    }
  };

  const openRejectDialog = (workout: WorkoutPlan) => {
  ...
  const stats = {
    pending: workouts.filter(w => w.approval_status === 'pending').length,
    approved: workouts.filter(w => w.approval_status === 'approved').length,
    rejected: workouts.filter(w => w.approval_status === 'rejected').length,
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/30">
      ...
      <div className="container mx-auto px-4 pt-28 py-8 pb-20 max-w-7xl space-y-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Gerencie e aprove treinos gerados pela IA</p>
          </div>
        </div>
            ...
            {loading ? (
              ...
              <Card>
                <CardContent className="pt-6 text-center text-muted-foreground">
                  ...
                  <p>Nenhum treino {activeTab === 'pending' ? 'pendente' : activeTab === 'approved' ? 'aprovado' : 'rejeitado'}</p>
                </CardContent>
              </Card>
            ) : (
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {workout.approval_status === 'pending' && (
                      <div className="flex gap-2 pt-2">
                        ...
                        <Button 
                          onClick={() => openRejectDialog(workout)}
                          className="flex-1"
                          variant="destructive"
                        >
      ...
      <AlertDialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Rejeitar Treino</AlertDialogTitle>
            <AlertDialogDescription>
              Por favor, informe o motivo da rejeição para que o aluno possa entender e gerar um novo treino.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <Textarea
            placeholder="Ex: Exercícios muito avançados para o nível do aluno..."
