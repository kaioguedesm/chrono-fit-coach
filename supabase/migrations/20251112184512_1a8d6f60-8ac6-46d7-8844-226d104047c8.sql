...
DROP POLICY IF EXISTS "Users can create their own pending signup" ON public.pending_personal_signups;
DROP POLICY IF EXISTS "Admins can view all pending signups" ON public.pending_personal_signups;

-- Remover trigger e função se existirem
DROP TRIGGER IF EXISTS on_email_confirmed_create_personal_role ON auth.users;
DROP FUNCTION IF EXISTS public.process_pending_personal_signup();

-- Remover tabela se existir
DROP TABLE IF EXISTS public.pending_personal_signups;

-- Criar tabela para armazenar solicitações pendentes de personal trainers
CREATE TABLE public.pending_personal_signups (
...
CREATE TRIGGER on_email_confirmed_create_personal_role
